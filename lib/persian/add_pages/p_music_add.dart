...
class pMusicAddPage extends StatelessWidget {
  const pMusicAddPage({super.key});
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: () {
          ...
          if (text.isEmpty) {
          ...
          } else if (Get.find<MusicTaskControllers>().isediting.value) {
            var task = Get.find<MusicTaskControllers>()
                .task[Get.find<MusicTaskControllers>().index];
            //
            ...
            Get.find<MusicTaskControllers>()
                .task[Get.find<MusicTaskControllers>().index] = task;
            Get.back();
          } else {
            Get.find<MusicTaskControllers>().task.add(
                  MusicTaskModels(
        ...
        style: TextButton.styleFrom(backgroundColor: KOrangeColor),
        child: Text(Get.find<MusicTaskControllers>().isediting.value
            ? 'ویرایش'
            : 'اضافه کردن'),
      ),
    );
