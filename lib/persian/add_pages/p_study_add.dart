...
class pStudyAddPage extends StatelessWidget {
  const pStudyAddPage({super.key});
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: () {
          ...
          } else if (Get.find<StudyTaskControllers>().isediting.value) {
            var task = Get.find<StudyTaskControllers>()
                .task[Get.find<StudyTaskControllers>().index];
            //
            task.title = Get.find<TextFieldControllers>().title!.text;
            task.subtitle = Get.find<TextFieldControllers>().subtitle!.text;
            //
            Get.find<StudyTaskControllers>()
                .task[Get.find<StudyTaskControllers>().index] = task;
            Get.back();
          } else {
            Get.find<StudyTaskControllers>().task.add(
                  StudyTaskModels(
        ...
        child: Text(Get.find<StudyTaskControllers>().isediting.value
            ? 'ویرایش'
            : 'اضافه کردن'),
      ),
    );
            ...
            children: [
              Expanded(
                child: Container(
                  ...
                  margin: const EdgeInsets.only(left: 236.0),
                  child: ElevatedButton(
          ...
          children: [
            Expanded(
              child: Container(
                ...
                child: Text(
                  Get.find<StudyTaskControllers>().isediting.value
                      ? 'ویرایش تسک'
                      : 'تسک جدید تکالیف',
