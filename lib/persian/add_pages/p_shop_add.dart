...
class pShopAddPage extends StatelessWidget {
  const pShopAddPage({super.key});
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: () {
          ...
          } else if (Get.find<ShopTaskControllers>().isediting.value) {
            var task = Get.find<ShopTaskControllers>()
                .task[Get.find<ShopTaskControllers>().index];
            //
            task.title = Get.find<TextFieldControllers>().title!.text;
            task.subtitle = Get.find<TextFieldControllers>().subtitle!.text;
            //
            Get.find<ShopTaskControllers>()
                .task[Get.find<ShopTaskControllers>().index] = task;
            Get.back();
          } else {
            Get.find<ShopTaskControllers>().task.add(
                  ShopTaskModels(
        ...
        style: TextButton.styleFrom(backgroundColor: KPhiroozeColor),
        child: Text(Get.find<ShopTaskControllers>().isediting.value
            ? 'ویرایش'
            : 'اضافه کردن'),
      ),
    );
          ...
          children: [
            Expanded(
              child: Container(
                ...
                child: Text(
                  Get.find<ShopTaskControllers>().isediting.value
                      ? 'ویرایش تسک'
                      : 'تسک جدید خرید',
